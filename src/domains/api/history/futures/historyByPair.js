...
const { getFuturesHistoryPair, getFuturesHistoryPairConditional } = require("../queries");
...
router.get("/last/:pair/from/:from", async (req, res) => {

  // function takes user id and pair given in request, returns user's historical positions
  const trades = await getFuturesHistoryPairConditional(req.user.id, req.params.pair, req.params.from);
